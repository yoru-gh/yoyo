...
var iSpeed = 166.6;
var iCurvature = 0.004;
if (viewWidth > 420 && viewWidth <= 768) {
	var iSpeed = 666.67;
	var iCurvature = 0.002;
} else if (viewWidth > 768) {
	var iSpeed = 1666.67;
	var iCurvature = 0.001;
}
// 为列表添加事件委托
list.touch(function(event){
	...
	if (target.nodeName.toLowerCase() == "span" || target.nodeName.toLowerCase() == "svg" || target.nodeName.toLowerCase() == "use") {
		var touchX = event.clientX || this.touches.X;
		var touchY = event.clientY || this.touches.Y;
		// 初始化提示点的坐标
		point.style.cssText = "top:"+touchY+"px;left:"+touchX+"px";
		// 抛物线函数
		// 提示元素(起始位置)/目标元素(终点位置)/各种设置
		parabola(point, fab, {
			speed: iSpeed, // 数值越大速度越快
			curvature: iCurvature, // 曲线的开口大小
			progress: function(){
				// 抛物运动时经过每一个点时触发
			},
			complete: function(){
...
// 2018-04-08
// 不兼容 IE9 以下浏览器
var parabola_old = function(element, target, options) {
	/*
	 * 网页模拟现实需要一个比例尺
